...
public:
    ...
    unique_ptr<FILE, FileCloser> open_file(const char* path, const char* mode)
    {
        ...
        if(!f) throw runtime_error("Example 1 failed! Failed to open file");
...
public:
    ...
    {
        explicit BadActor(string name, CallbackEvent& event) :
         name_(std::move(name)), 
         event_(event){}
    ...
    {
        explicit NiceActor(string name, CallbackEvent& event) :
         name_(std::move(name)), 
         event_(event){}
